...
Add:自动爬塔  
自己调整游戏编队界面的自动编队设置,推荐的选项是仅从者,4或5名,234星或15星  
Fix:guardian  
...
Fix:Saber->剑士  
对于非简体中文语言的每周任务适配,只需要在每个目标中重复最后一个标签的不同语言版本就行,应该不存在一种语言的标签是另一种语言的另一种标签的情形  
Opt:移除了每周任务求解器自身的输出  

## 2024/01/09 v19.1.4

Fix:map entrance 2-1,3-1  
制作3-1资源的时候文件名写成了2-1  

## 2024/01/08 v19.1.3

Opt:打开宝物库之门  
正月活动迦勒底之门全职阶关卡常驻列表太长扒拉不到  
在使用显微镜观察标本时,应先转动粗准焦螺旋,再调节细准焦螺旋  
...
Fix(?):我发现我写的AlasFpyBridge有个bug,在计数特殊掉落时访问了一个不存在的成员,主进程出错而FGO-py子进程仍存在,最终子进程stdout缓冲写满阻塞,因此改变了一下特殊掉落的文本使得特殊掉落计数不会触发  
Alas那边等我自动每周任务写好之后一起整个大的  
...
Del:南溟弓张八犬传活动收菜  
Fix:多游戏服务器适配与直接在手机上运行的连接设备冲突  
Add:cli下connect命令--sleep参数  
有点扭曲,但是稳  
...
Rem:本版本进行了较大幅度的更新,源码有大量更改,尽管已经经过测试,但仍不能保证相对较少的bug  
Rem:本次更新后fgoTeamup.ini配置文件与先前的不兼容  
...
警告:现在不会区分友情池和氪金池!!!  
Rem:出于账户资产安全考虑,你需要先抽一次十连,待进入转圈界面时再启动程序  
...
Rem:由于这些更改会使得战斗行为不完全依照您的设置,所以现在`skillInfo[i][j][0]`和`houguInfo[i][0]==0`时代表不使用技能/宝具  
8说了,我去冬木大桥刷凶骨了  
